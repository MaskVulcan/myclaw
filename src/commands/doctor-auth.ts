...
function buildCodexProviderOverrideWarning(providerOverride: unknown): string {
  ...
  if (isRecord(providerOverride)) {
    if (typeof providerOverride.api === "string") {
      lines.push(`- models.providers.${CODEX_PROVIDER_ID}.api=${providerOverride.api}`);
    }
    if (typeof providerOverride.baseUrl === "string") {
      lines.push(`- models.providers.${CODEX_PROVIDER_ID}.baseUrl=${providerOverride.baseUrl}`);
    }
  }
  lines.push(
