...
export function buildStatusSessionsRows(params: {
  ...
  return params.recent.flatMap((sess) => {
    const baseRow = {
    ...
    const cacheDetails = params.formatPromptCacheCompact(sess) || params.muted("—");
    const cacheParts =
      cacheDetails === params.muted("—")
        ? [cacheDetails]
        : cacheDetails
            .split(" · ")
            .map((part) => part.trim())
    ...
    return [
      {
        ...baseRow,
        Cache: cacheParts[0] ?? params.muted("—"),
      },
      ...cacheParts.slice(1).map((part) => ({
        Key: "",
        Kind: "",
        Age: "",
        Model: "",
