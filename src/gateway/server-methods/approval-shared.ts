...
type RequestedApprovalEvent<TPayload> = {
  id: string;
  request: TPayload;
  createdAtMs: number;
  expiresAtMs: number;
...
export function resolvePendingApprovalRecord<TPayload>(params: {
  ...
  if (resolvedId.kind === "prefix" && params.allowPrefixLookup === false) {
...
export async function handlePendingApprovalRequest<TPayload>(params: {
  manager: ExecApprovalManager<TPayload>;
...
export async function handleApprovalResolve<TPayload, TResolvedEvent extends object>(params: {
  ...
  const resolved = resolvePendingApprovalRecord({
    ...
    exposeAmbiguousPrefixError: params.exposeAmbiguousPrefixError,
    allowPrefixLookup: params.allowPrefixLookup,
  });
  if (!resolved.ok) {
