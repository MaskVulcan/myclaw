...
- `worktree?` (`off|git`, default `off`; `git` creates a detached worktree for the spawned child when the inherited workspace is inside a git repo; unsupported for `runtime: "acp"`)
