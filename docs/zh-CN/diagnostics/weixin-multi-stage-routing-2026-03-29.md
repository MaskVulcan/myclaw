...
- `kimi-cli` prompt 占位问题修复后，微信首轮 fast pass 已能稳定命中
- 但真实简单消息仍常见约 6 到 7 秒延迟
- 部分明显复杂的问题还会先绕一圈 fast pass，再升级到强模型

本轮收紧：

- fast-pass 系统提示改为允许处理“直接、简单、单步”的请求
- 保留简洁输出约束，但不再把 fast-pass 限死为只回问候/致谢
- 网关本地改成黑名单式复杂度短路，只拦明显复杂项：
  - 多行消息
  - 带结构化片段 / 链接 / markdown
  - 明显是调试 / 排查 / 设计 / 优化 / 同步 / 重启 / 提交类动作
  - 明显是性能 / 配置 / 日志 / 异常 / context / memory / session 相关复杂追问
  - 明显依赖前文的操作型 follow-up（如“继续”“我又发了”“看看日志”）
  - 以上都会直接跳过 fast pass，进入强阶段
- `kimi-cli` 默认参数进一步压缩为：
  - `--quiet`
  - `--no-thinking`
...
- 普通简单问答、短命令意图、轻量单步任务仍可走 `kimi-k2.5` 快路径
- 复杂、模糊、需要上下文的问题会直接切到 `codex-vip/gpt-5.4 xhigh`
- 避免复杂消息先命中快模型再回退，减少一段不必要的额外等待
