...
function parseApiJson<T>(rawText: string, label: string): T {
  try {
    return JSON.parse(rawText) as T;
  } catch (err) {
    throw new Error(`${label}: invalid JSON response: ${String(err)}`);
  }
}

function assertApiRetOk<T extends ApiStatusResponse>(rawText: string, label: string): T {
  const resp = parseApiJson<T>(rawText, label);
  if (typeof resp.ret === "number" && resp.ret !== 0) {
    const errmsg =
      typeof resp.errmsg === "string" && resp.errmsg.trim().length > 0
        ? ` errmsg=${resp.errmsg.trim()}`
        : "";
    throw new Error(`${label} failed: ret=${resp.ret}${errmsg}`);
  }
  return resp;
}
...
export async function sendMessage(
  ...
  assertApiRetOk<SendMessageResp>(rawText, "sendMessage");
}
...
export async function sendTyping(
  ...
  assertApiRetOk<SendTypingResp>(rawText, "sendTyping");
}
