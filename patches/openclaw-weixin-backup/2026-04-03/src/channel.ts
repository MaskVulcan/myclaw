...
function resolveOutboundContextToken(accountId: string, to: string): string | undefined {
  const info = inspectContextToken(accountId, to);
  const aLog = logger.withAccount(accountId);

  if (info.state === "restored") {
    aLog.info(
      `outbound context: restored persisted contextToken for to=${to} from disk before send`,
    );
  } else if (info.state === "persisted-mismatch") {
    aLog.warn(
      `outbound context: persisted contextToken exists for to=${to} but could not be restored into memory`,
    );
  } else if (info.state === "missing") {
    aLog.warn(`outbound context: no contextToken available for to=${to}`);
  } else {
    aLog.debug(`outbound context: using in-memory contextToken for to=${to}`);
  }
  ...
  outbound: {
    ...
    sendText: async (ctx) => {
      ...
      const result = await sendWeixinOutbound({
        ...
        text: ctx.text,
        accountId,
        contextToken: resolveOutboundContextToken(accountId!, ctx.to),
      });
      return result;
    },
    sendMedia: async (ctx) => {
      ...
      if (mediaUrl && (isLocalFilePath(mediaUrl) || isRemoteUrl(mediaUrl))) {
        ...
        const contextToken = resolveOutboundContextToken(account.accountId, ctx.to);
      ...
      const result = await sendWeixinOutbound({
        ...
        text: ctx.text ?? "",
        accountId,
        contextToken: resolveOutboundContextToken(account.accountId, ctx.to),
      });
      return result;
    },
  },
