...
export async function processOneMessage(
  ...
  try {
    ...
    if (contextToken && (full.from_user_id ?? "").trim()) {
      try {
        await flushPendingRemindersForRecipient({
          accountId: deps.accountId,
          to: full.from_user_id ?? "",
          opts: {
            baseUrl: deps.baseUrl,
        ...
        logger.error(
          `flushPendingRemindersForRecipient: failed accountId=${deps.accountId} to=${full.from_user_id ?? ""} err=${String(flushErr)}`,
        );
      }
    }
